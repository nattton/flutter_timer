...
void main() {
  ...
  setUpAll(() {
    registerFallbackValue(const Started(duration: 0));
  });
  ...
  group('TimerView', () {
    testWidgets('renders initial Timer view', (tester) async {
      when(() => timerBloc.state).thenReturn(const Initial(60));
    ...
    testWidgets('renders pause and reset button when timer is in progress',
        (tester) async {
      when(() => timerBloc.state).thenReturn(const RunInProgress(59));
      await tester.pumpTimerView(timerBloc);
    ...
    testWidgets('renders play and reset button when timer is paused',
        (tester) async {
      when(() => timerBloc.state).thenReturn(const RunPause(600));
      await tester.pumpTimerView(timerBloc);
    ...
    testWidgets('renders replay button when timer is finished', (tester) async {
      when(() => timerBloc.state).thenReturn(const RunComplete());
      await tester.pumpTimerView(timerBloc);
    ...
    testWidgets('timer started when play arrow button is pressed',
      ...
      verify(
        () => timerBloc.add(
          any(
            that: isA<Started>().having((e) => e.duration, 'duration', 60),
          ),
        ),
      ).called(1);
    });
    ...
    testWidgets(
      ...
      when(() => timerBloc.state).thenReturn(const RunInProgress(30));
      await tester.pumpTimerView(timerBloc);
      await tester.tap(find.byIcon(Icons.pause));
      verify(() => timerBloc.add(const Paused())).called(1);
    });
    ...
    testWidgets(
      ...
      verify(() => timerBloc.add(const Reset())).called(1);
    });

    testWidgets(
        'timer resumes when play arrow button is pressed '
        'while timer is paused', (tester) async {
      when(() => timerBloc.state).thenReturn(const RunPause(30));
      await tester.pumpTimerView(timerBloc);
      await tester.tap(find.byIcon(Icons.play_arrow));
      verify(() => timerBloc.add(const Resumed())).called(1);
    });
    ...
    testWidgets('actions are not rebuilt when timer is running',
      ...
      whenListen(
        timerBloc,
        controller.stream,
        initialState: const RunInProgress(10),
      );
      await tester.pumpTimerView(timerBloc);
      ...
      controller.add(const RunInProgress(9));
      await tester.pump();
